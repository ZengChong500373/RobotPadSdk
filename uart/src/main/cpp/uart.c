...
void *uart_recv(void *arg) {
    ...
    while (uart->running) {
        ...
        if ((revlen = read(uart->fd, revbuff, sizeof(revbuff))) == -1) { DBG_LOG_ERROR(
                    "uart_recv error:%d", errno);
            continue;
        }
...
//uart初始化
int uart_init(UART_HANDLE *uart_hd, const char *device, int rate, uart_rec_fn uart_rec_cb,
              void *user_data) {
    int ret = 0;
    uartData *uart = NULL;
    void *unused;
    pthread_attr_t attr;DBG_LOG_TRACE("uart_init(%s,%x,%x)[in]", device, uart_rec_cb, user_data);
    //句柄检查
    if (NULL == device || NULL == uart_rec_cb || NULL == uart_hd) { DBG_LOG_ERROR(
                "NULL == device || NULL == uart_read_cb");
        return -1;
    }
    //申请结构体
    ...
    if (uart->fd < 0) { DBG_LOG_ERROR("open failed: %s", device);
        ret = -1;
        goto error;
    }
    //设定属性
    ...
    if (0 != ret) { DBG_LOG_ERROR("uart recv thread create failed!\n");
        ret = -1;
        goto error;
    }
    *uart_hd = uart;
    ...
    exit:DBG_LOG_TRACE("uart_init()%d[out]", ret);
    return ret;
}
...
int uart_send(UART_HANDLE uart_hd, const char *msg, int msglen) {
    ...
    if (0) {
        int i = 0;
        for (i = 0; i < msglen; i++) { DBG_LOG_DEBUG("%x ", msg[i]);
        }DBG_LOG_DEBUG("---send\n");
    }
    //反复发送，直到全部发完
    while (sended_len != msglen) {
        int retlen = 0;
        retlen = write(uart->fd, msg + sended_len, msglen - sended_len);DBG_LOG_DEBUG(
                "uart write msg, len:%d\n", retlen);
        if (retlen < 0) { DBG_LOG_ERROR("uart_send failed! ret=%d\n", ret);
            return -1;
        }
        sended_len += retlen;
    }
...
void uart_uninit(UART_HANDLE uart_hd) {
    void *unused;DBG_LOG_TRACE("uart_uninit(%x)[in]", uart_hd);
    if (NULL != uart_hd) {
        uartData *uart = (uartData *) uart_hd;
        //关闭运行状态
        uart->running = 0;
