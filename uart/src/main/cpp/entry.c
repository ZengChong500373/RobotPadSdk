...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if ((*tempEnv)->RegisterNatives(tempEnv, cls, native_methods,
                                    sizeof(native_methods) / sizeof(native_methods[0])) < 0) {
        return JNI_ERR;
    }

    connector_class = (*tempEnv)->NewGlobalRef(tempEnv, cls);
    connector_recv_method = (*tempEnv)->GetStaticMethodID(tempEnv, connector_class, "onReceive",
                                                          "([B)V");
...
void uart_rec(const void *msg, unsigned int msglen, void *user_data) {
    ...
    (*rec_thread_env)->CallStaticVoidMethod(rec_thread_env, connector_class, connector_recv_method,
                                            rec_data);
